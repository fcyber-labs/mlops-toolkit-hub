...
def display_metric_row(metrics, phase_name):
    ...
    exclude_keys = ['tn', 'fp', 'fn', 'tp', 'n_total', 'n_pos', 'lift_by_decile', 
...
def display_additional_metrics(metrics, phase_name):
    ...
    if 'lift_by_decile' in metrics and metrics['lift_by_decile']:
        ...
        fig.add_hline(y=1.0, line_dash="dash", line_color="gray", 
                     annotation_text="Random baseline", annotation_position="right")
        fig.update_layout(
...
if ref_data_1 is not None and stream_data_1 is not None:
    ...
    if len(drift_df_1) > 0:
        ...
        fig1 = go.Figure()
        colors = ['#2ecc71' if psi < 0.1 else '#f39c12' if psi < 0.25 else '#e74c3c' 
        ...
        fig1.add_hline(y=0.1, line_dash="dash", line_color="green", 
                      annotation_text="Warning (0.1)", annotation_position="right")
        fig1.add_hline(y=0.25, line_dash="dash", line_color="red", 
...
if ref_data_2 is not None and stream_data_2 is not None:
    ...
    if len(drift_df_2) > 0:
        ...
        st.subheader("📈 PSI Drift Visualization")
        fig2 = go.Figure()
        colors_2 = ['#2ecc71' if psi < 0.1 else '#f39c12' if psi < 0.25 else '#e74c3c' 
        ...
        fig2.add_hline(y=0.1, line_dash="dash", line_color="green", 
                      annotation_text="Warning (0.1)", annotation_position="right")
...
st.caption(f"🔄 Production Monitoring Dashboard | PSI + KS Drift Detection | Last updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
st.caption("💡 PSI is the industry standard for drift detection - used by banks, fintechs, and financial institutions worldwide")