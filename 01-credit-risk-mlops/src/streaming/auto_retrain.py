...
class AutoRetrainer:
    ...
    def retrain_model(self):
        ...
        with mlflow.start_run(run_name=f"auto_retrain_{datetime.now().strftime('%Y%m%d_%H%M%S')}"):
            ...
            mlflow.log_metric("features_with_drift", self.detector.drift_report.get('features_with_drift', 0))
        ...
        if result.returncode == 0:
            print(f"✅ Auto-retraining completed successfully!")
            ...
            print(f"❌ Auto-retraining failed!")
            print(result.stderr)
