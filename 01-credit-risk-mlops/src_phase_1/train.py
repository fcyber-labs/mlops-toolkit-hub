...
def train(model_path: str):
    ...
    lgb_recall = LGBMClassifier(
        ...
        num_leaves=50, min_child_samples=5, subsample=0.8,
        colsample_bytree=0.7, reg_alpha=0.1, reg_lambda=0.1,
        scale_pos_weight=10.0, boosting_type="gbdt",
        random_state=RANDOM_STATE, verbosity=-1, n_jobs=-1,
    )
    lgb_recall.fit(X_resampled, y_resampled)
    logger.info("  LightGBM (recall maximizer) ✓")

    xgb_final_params = {k: v for k, v in best_xgb_params.items()
    ...
    rf_model = RandomForestClassifier(
        ...
        class_weight={0: 1, 1: 5}, random_state=RANDOM_STATE, n_jobs=-1,
    )
    ...
    plt.axvline(x=final_metrics["recall"], color="red", ls=":", alpha=0.7,
                label=f"Op. point (t={final_threshold:.3f})")
    plt.axhline(y=MIN_PRECISION, color="orange", ls=":", alpha=0.7,
    ...
    for name, model in [
        ...
        ("xgb", xgb_opt), ("rf", rf_model), ("et", et_model),
    ]:
    ...
    model_metadata = {
        ...
        "final_metrics":     {k: round(v, 4) for k, v in final_metrics.items()
                              if isinstance(v, float)},
        "default_metrics":   {k: round(v, 4) for k, v in results_default.items()
                              if isinstance(v, float)},
        "lgb_best_params":   best_lgb_params,
        "xgb_best_params":   {k: v for k, v in best_xgb_params.items()
                              if k != "early_stopping_rounds"},
    ...
    with mlflow.start_run(run_name="soft_voting_recall_v3") as run:
        ...
        mlflow.log_params(
            {f"lgb_{k}": v for k, v in best_lgb_params.items()
        ...
        for prefix, res in [("recall_opt", results_recall),
            ...
            mlflow.log_metrics(
                {f"{prefix}_{k}": round(v, 4) for k, v in res.items() if isinstance(v, float)}
            )

        # text artifacts
        cm = confusion_matrix(y_test, (y_pred_proba_ensemble >= final_threshold).astype(int))
        cm_df = pd.DataFrame(cm, columns=["Pred Good", "Pred Bad"],
                             index=["Act Good", "Act Bad"])
        ...
        # plot artifacts
        for fname in ["reports/roc_curve.png", "reports/pr_curve.png",
